...
function deploy(overrides = {}, fileConfig = false, syncOnly = false) {
  ...
  'cloudformation describe-stacks')
    ...
    case "$*" in *BucketName*) echo "\${MOCK_BUCKET:-example-harness-site}" ;; *) echo "\${MOCK_DIST:-E123EXAMPLE}" ;; esac ;;
esac`,
    };
...
test('loads ignored deployment configuration and uses the deployed outputs', () => {
  ...
  assert.match(result.calls, /s3 sync dist\/ s3:\/\/example-harness-site/);
...
test('dirty trees, feature branches, and mismatched submodules stop deployment', () => {
  for (const options of [{ MOCK_DIRTY: ' M README.md' }, { MOCK_BRANCH: 'feature' }, { MOCK_SUBMODULE: '+different vendor/agent-harness' }]) {
...
test('CI sync rejects absent or malformed destination identifiers before AWS calls', () => {
  for (const values of [
    ...
    { BUCKET: 'example-harness-site', DIST_ID: 'None' },
    { BUCKET: 'example-harness-site/path', DIST_ID: 'E123EXAMPLE' },
  ]) {
